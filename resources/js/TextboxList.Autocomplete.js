...
$.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
	...
	var search = function(bit){
		...
		if (options.queryRemote){
			if (searchValues[search]){
				...
				currentRequest = $.ajax({
					url: options.remote.url + "/" + search,
					/*data: data,*/
					dataType: 'json',
					success: function(r){
						searchValues[search] = r.matches;
						values = r.matches;
						showResults(search);
					}
				});
	...
	var addResult = function(r, searched){
		...
		//var element = $('<li class="'+ prefix +'-result" />').html(r.value).data('textboxlist:auto:value', r);		
	...
	var addCurrent = function(){
		...
		//var b = textboxlist.create('box', value.caption);
		//console.log(b, index);
		if (b){
			b.autoValue = value;
